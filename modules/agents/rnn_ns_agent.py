...
class RNNNSAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state, dormant_record):
        hiddens = []
        qs = []
        if inputs.size(0) == self.n_agents:
            for i in range(self.n_agents):
                q, h = self.agents[i](inputs[i].unsqueeze(0), hidden_state[:, i], dormant_record)
                hiddens.append(h)
                qs.append(q)
            if dormant_record is not None:
                raise NotImplementedError('Dormant not implemented yet.')
            return th.cat(qs), th.cat(hiddens).unsqueeze(0)
        else:
            for i in range(self.n_agents):
                inputs = inputs.view(-1, self.n_agents, self.input_shape)
                q, h = self.agents[i](inputs[:, i], hidden_state[:, i], dormant_record)
