...
class MASIAAgent(nn.Module):
    ...
    def __init__(self, input_shape, args):
        ...
        self.encoder = state_enc_REGISTRY[args.state_encoder](input_shape=input_shape, output_shape=state_dim,
                                                              n_agents=args.n_agents, latent_dim=args.state_repre_dim,
                                                              args=args)
    ...
    def forward(self, inputs, hidden_state,  dormant_record: Optional[Dict], encoder_hidden_state):
        assert dormant_record is None, 'Not supported now'
        ...
        if "vae" in self.args.state_encoder:
            ...
            if self.args.noise_env and self.args.noise_type == 0:
                ...
                inputs = inputs.reshape(bs, self.args.n_agents, inputs.shape[-1]).unsqueeze(1).repeat(1,
                                                                                                      self.args.n_agents,
                                                                                                      1, 1)
        ...
        if self.args.noise_env and self.args.noise_type == 1:
            ...
            repeated_z = z.unsqueeze(1).repeat(1, self.args.n_agents, 1).reshape(bs * self.args.n_agents,
                                                                                 -1)  # [bs*n_agents, state_repre_dim]
    ...
    def rl_forward(self, inputs, state_repr, hidden_state):
        ...
        # through gate
        weighted = F.sigmoid(self.gate(inputs))  # [bs*n_agents, state_repre_dim]
        repeated_z = state_repr.unsqueeze(1).repeat(1, self.args.n_agents, 1).reshape(bs * self.args.n_agents,
                                                                                      -1)  # [bs*n_agents, state_repre_dim]
        weighted_z = weighted * repeated_z
