...
class PreprocessManager:
    ...
    def __init__(self, n_agents: int, preprocess_desc: Optional[str], args,
                 ...
                 cityflow_adjacency: Optional[Dict[str, List[str]]] = None):
        """
        ...
        self.cityflow_adjacency = cityflow_adjacency
    ...
    def get_schedule_func(self) -> Optional[Callable[[int], str]]:
        ...
        if self.adaptive_worker is not None:
            return None
        # revised elif self.preprocess_env_type in ['lbf', 'resco', 'rware', 'asc2', 'metadrive', 'resco2']:
    ...
    def get_next_visibility_str(self, t_env: int) -> Optional[str]:
        ...
        # revised if self.preprocess_env_type in ['lbf', 'rware', 'asc2', 'metadrive']:
    ...
    def _get_one_preprocessed_obs(self, obs, visible_str):
        ...
        if self.preprocess_env_type == 'cityflow':
            processed = cityflow_topo_preprocess(original_obs=obs_to_preprocess,
                                                adjacency= self.cityflow_adjacency,
                                                sight=int(visible_str[:-1]))

        elif self.preprocess_env_type == 'lbf':
