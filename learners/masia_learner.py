...
class MASIALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, assigned_obs_key: Optional[str] = None):
        # TODO: episode_num: int is not used may be removed
        # Representation learning training
        repr_loss = self.repr_train(batch, t_env, episode_num=None, assigned_obs_key=assigned_obs_key)
        # RL training
        self.rl_train(batch, t_env, repr_loss=repr_loss, episode_num=None, assigned_obs_key=assigned_obs_key)

    def repr_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, assigned_obs_key: Optional[str] = None):
        ...
        for t in range(batch.max_seq_length):
            recons_t, _, z_t = self.mac.vae_forward(batch, t, obs_key=cur_obs_key)
        ...
        loss_dict = self.mac.agent.encoder.loss_function(recons.reshape(bs * seq_len, -1),
        ...
        if self.args.use_latent_model:
            ...
            with th.no_grad():
                ...
                for t in range(batch.max_seq_length):
                    target_projected_t = self.mac.target_transform(batch, t, obs_key=cur_obs_key)
    ...
    def rl_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, repr_loss,
                 assigned_obs_key: Optional[str] = None):
        # Obs key for this batch
        if assigned_obs_key is not None:
            cur_obs_key = assigned_obs_key
        else:
        ...
        self.mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            state_repr_t = self.mac.enc_forward(batch, t=t, obs_key=cur_obs_key)
            if not self.args.rl_signal:
                state_repr_t = state_repr_t.detach()
            agent_outs = self.mac.rl_forward(batch, state_repr_t, t=t, obs_key=cur_obs_key)
        ...
        for t in range(batch.max_seq_length):
            state_repr_t = self.target_mac.enc_forward(batch, t=t, obs_key=cur_obs_key)
            target_agent_outs = self.target_mac.rl_forward(batch, state_repr_t, t=t, obs_key=cur_obs_key)
    ...
    def load_models(self, path):
        ...
        self.latent_model.load_state_dict(
            th.load("{}/latent_model.th".format(path), map_location=lambda storage, loc: storage))
