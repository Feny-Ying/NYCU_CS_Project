...
class PPOLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # The only supported preprocess type is "ucb"
        # So, if the preprocess type is not "ucb", raise an error (e.g., "equal")
        if self.args.preprocess_desc is not None:
            assert 'ucb(' in self.args.preprocess_desc

        # Some functions are not supported here
        assert (not self.args.policy_diff_interval_1 and
                not self.args.policy_diff_interval_2 and
                not self.args.policy_diff_interval_3 and
                not self.args.dormant_check_interval)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, assigned_obs_key: Optional[str] = None):
        ...
        for t in range(batch.max_seq_length - 1):
            agent_outs = self.old_mac.forward(batch, t=t, dormant_record=None, obs_key=cur_obs_key)
